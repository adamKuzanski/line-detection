...
# %%
# STEP 3 - APPLY GAUSSIAN SMOOTHING / BLURRING
kernel_size = 5
...
low_treshold = 70
high_treshold = 140
canny_edges = canny(blurred, low_treshold, high_treshold)
_ = plt.imshow(canny_edges, cmap='gray')


# %%
# STEP 5 - CREATE A MASKED EDGES IMAGE 
...
# %%
#STEP 6 - CREATE A FOUR SIDED POLYGON TO THE MASK
image_shape = image.shape
vertices = np.array([[(0, image_shape[0]), (450, 300), (540, 300), (image_shape[1], image_shape[0])]], dtype=np.int32)
polygon = region_of_interest(image, vertices)
plt.imshow(polygon)


# %%
# STEP 7 - JOIN MASK AND CANNY EDGES IMAGE
cv2.fillPoly(mask, vertices, ignore_mask_color)
...
treshold = 30           # minimum number of votes (intersections in Hough grid cell)
min_line_length = 50    # minimum number of pixels making up a line
max_line_gap = 20       # maximum gap in pixels between connectable line segments

lines = hough_lines(masked_edges, rho, theta, treshold, min_line_length, max_line_gap)
plt.imshow(lines)
...
for image_name in images_list:
    ...
    cv2.imwrite('test_images_output/{}'.format(image_name), lines)
